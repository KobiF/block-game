...
function draw(obj){
  ...
  if(obj.type=="player"){
    ctx.fillRect(((Math.floor(obj.x*10)/10) *cellSize) + 0.5,((Math.floor(obj.y*10)/10) *cellSize)+0.5,parseInt(cellSize,10) -1,parseInt(cellSize)-0.5);
  }else{
...
function right(){
  if(player.Xspeed<maxHorzSpeed){
    player.Xspeed += horzSpeed;
  }
}
...
function phisics(){
  ...
  if(Math.abs(player.Xspeed)<0.01)player.Xspeed = 0;
  if(canWallJump){
